...
import {getModelsByQuery} from "@/services/CivitAIService";
...
<template>
  <div>
    ...
    <ModelSearchControls @update:filters="updateFilters" :loading="isLoading" />
    <div v-if="isLoading" class="flex justify-center items-center h-32">
      <span class="loading loading-spinner loading-lg"></span>
    </div>

    <div v-else-if="models.length > 0" class="mt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 3xl:grid-cols-6 gap-5">
      <ModelCard v-for="model in models" :key="model.id" :model="model" :blur-n-s-f-w="!filters.nsfw" :downloads-enabled="downloadsEnabled" />
    </div>

    <p v-else class="text-center text-xl font-bold absolute top-1/2 bottom-1/2 w-full">Sorry, no models could be found!</p>
  </div>
</template>
