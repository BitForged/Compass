      ...
      <div class="col-span-12 md:col-span-9 max-md:pb-5">
        <div class="prompt-container col-span-12 md:col-span-9 order-1">
          <div class="form-control border-2 border-opacity-90 border-zinc-700 cornered">
            ...
            <textarea ref="negativePromptArea" id="negativePrompt" @change="adjustHeight($event.target)" @keyup="adjustHeight($event.target)" :disabled="recalledImageId !== null || imageParams.options.cfg_scale === 1.0" v-model="imageParams.options.negative_prompt" class="textarea h-24 textarea-bordered m-3" placeholder="Optionally, enter a negative prompt - which describes what you don't want to see in the image."></textarea>
            <span v-if="imageParams.options.cfg_scale === 1.0" class="m-3 mt-1 text-sm text-error">Negative Prompts are ignored when CFG Scale is 1. To use a Negative Prompt, adjust your CFG Scale.</span>
          ...
          <div class="form-control cornered" :class="{'glow': (lastJob && lastJob.status === 'completed')}">
            ...
            <div class="m-3">
              ...
              <div class="grid grid-cols-12 gap-2 mt-2">
                <button v-if="!img2imgParams.inpainting" :disabled="isWorking || !lastJob || lastJob.status !== 'completed'" @click="onVariationClick(0.3)" class="btn btn-accent col-span-12 xl:col-span-6 relative glow-hover"><oh-vue-icon class="size-6" animation="pulse" name="gi-perspective-dice-six-faces-random"/>&nbsp; Variation (Subtle)</button>
                <button v-if="!img2imgParams.inpainting" :disabled="isWorking || !lastJob || lastJob.status !== 'completed'" @click="onVariationClick(0.7)" class="btn btn-accent col-span-12 xl:col-span-6 relative glow-hover"><oh-vue-icon class="size-6" animation="pulse" name="gi-perspective-dice-six-faces-random"/>&nbsp; Variation (Strong)</button>
              </div>
